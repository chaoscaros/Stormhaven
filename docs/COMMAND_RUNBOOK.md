...
70. 按 Tab 打开背包，选择一个物品后点击快捷栏 `1`–`8` 中任意槽位；关闭菜单后对应槽应显示物品图标和当前数量，且不会自动触发 Item Use。
71. 在背包快捷栏编辑区点击已占用槽位可覆盖原有绑定；点击该槽右侧 `×` 可独立清空，其他槽位保持不变。
72. 按 B 打开建造页，选择木制地基、木制墙体或篝火后点击任意快捷栏槽；关闭菜单后按对应数字键应进入该建筑的放置模式。
73. 在 Player Menu 内点击绑定或清空时，不应进入 BuildPlacement、锁定鼠标或产生 Gameplay 行为；菜单继续允许鼠标交互。
74. 刷新页面后快捷栏恢复初始前三格建筑、其余为空，这是当前未实现快捷栏持久化的预期行为。
