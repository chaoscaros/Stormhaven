...
当前已完成现代桌面浏览器技术基础、与渲染解耦的时间/天气 Domain，以及独立 Weather Presentation Layer：
...
当前阶段明确不包含天气音频、室内雪遮罩、Temperature、Wetness、物品数据、制作配方、生存计算、存档结构或建筑放置行为。Weather Presentation 只改变画面，不改变任何玩法数值。
