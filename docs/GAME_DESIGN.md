...
当前木制地基表现已进入 Blender Foundation Pilot：独立板材、细板缝、承重边框与
横梁、克制倒角和哑光木纹；保持 2 × 0.2 × 2 米模块及原建造/碰撞/存档规则。
同一 `.blend` 提供世界模型与 UI 缩略图。当前是待用户视觉验收的首个模型，
不是其它 P0 重做或新玩法授权。
...
- 地基、墙与篝火不得与未拾取物资的占位体积重叠；显示“此处有未拾取物资，请先拾取”且不扣材料。部分拾取仍占位，全部拾取后释放；不自动删除物资，也不让小物资阻挡玩家移动。旧存档中已有重叠不自动拆除。
