...
describe("Save presentation rebuild (headless, not visual acceptance)", () => {
  it("rebuilds collision, snow obstacles, pickup lookup, fire light/heat and interaction without duplicates", async () => {
    ...
    const buildings = new BuildingPresentation(scene, target.gameplay.buildCatalog, obstacles, new CampfireBuildingBinding(target.gameplay.campfireSystem));
    const pickups = new WorldPickupPresentation(scene, target.gameplay.pickupPlacements, target.gameplay.itemCatalog);
    ...
    try {
      ...
      const counts = [scene.meshes.length, scene.lights.length, obstacles.getAll().length];
      expect(await service.load()).toMatchObject({ reason: "ok" });
      expect([scene.meshes.length, scene.lights.length, obstacles.getAll().length]).toEqual(counts);
      expect(target.gameplay.campfireSystem.getAll()).toHaveLength(1);
    } finally { pickups.dispose(); buildings.dispose(); scene.dispose(); engine.dispose(); }
  });
});
